package Tools::Log::Item;

use strict;
use Tools::Check qw[check];
use Tools::Log::Handlers;
...
sub AUTOLOAD {
    ...
    {   no strict 'refs';
        return *{"Tools::Log::Handlers::${AUTOLOAD}"}->(@_);
    }
}
...
=head1 SYNOPSIS
    ...
    # Implicitly used by Tools::Log to create Tools::Log::Item objects
    ...
    # See the Tools::Log::Handlers manpage for documentation on what
    # handlers are available by default and how to add your own


=head1 DESCRIPTION

Tools::Log::Item is a class that generates generic Log items.
These items are stored on a Tools::Log stack, so see the Tools::Log
manpage about details how to retrieve them.

You should probably not create new items by yourself, but use the
storing mechanism provided by Tools::Log.
...
documented in the Tools::Log::Handlers manpage.
...
Returns a reference to the Tools::Log object that stored this item.
This is usefull if you want to have access to the full stack in a
handler.

=head1 SEE ALSO

L<Tools::Log>, L<Tools::Log::Handlers>, L<Tools::Log::Config>
