...
use Tools::Log::Item;
use Tools::Log::Config;

local $Tools::Check::VERBOSE = 1;
...
=head1 SYNOPSIS
    use Tools::Log private => 0, config => '/our/cf_file/';
...
Tools::Log is a generic message storage mechanism.
...
Tools::Log also provides a powerfull way of searching through items
by regexes on messages, tags and level.

=head1 Hierarchy

There are 4 modules of interest when dealing with the Tools::Log::*
modules:
...
Tools::Log provides a few methods to manipulate the stack it keeps.
...
See the Tools::Log::Item manpage to see how to extract this meta-data
and how to work with the Item objects.
...
=item Tools::Log::Handlers

These are a collection of handlers that will be called for a level
that is used on a Tools::Log::Item object.
For example, if a message is logged with the 'carp' level, the 'carp'
handler from Tools::Log::Handlers will be called.
See the Tools::Log::Handlers manpage for more explenation about how
handlers work, which one are available and how to create your own.

=item Tools::Log::Config

Per Tools::Log object, there is a configuration required that will
...
Tools::Log::Config handles the creation of these configurations.
...
As a configuration file when you C<use Tools::Log>

=item *

As arguments when you C<use Tools::Log>

=item *

As a configuration file when you create a new Tools::Log object.
(The config will then only apply to that object if you marked it as
private)

=item *

As arguments when you create a new Tools::Log object.

You should never need to use the Tools::Log::Config module yourself,
as this is transparently done by Tools::Log, but it's manpage does
...
When using Tools::Log, or creating a new Tools::Log object, you can
...
See the manpage of Tools::Log::Config for the required format
...
If you choose to create shared objects, all Tools::Log objects will
use the same stack.
...
Tools::Log makes use of another module in the Tools::* series called
Tools::Check, which is a lightweight, yet powerful input checker and
parser. (See the Tools::Check manpage for details).
...
silently returns undef, at which point Tools::Log will generate a
warning.
...
config, nor any specific arguments supply a tag, then Tools::Log will
set it to 'NONE'
...
like C<tag>, Tools::Log will provide a default (which is 'log') if
neither your config file, nor any explicit arguments are given to
override it.

See the Tools::Log::Handlers manpage to see what handlers are
...
The default setting provided by Tools::Log is '0': do not remove.
...
sub import {
    ...
    $CONFIG = new Tools::Log::Config( %hash )
                or die qq[Problem initialising ] . __PACKAGE__;
...
sub new {
    ...
    my $conf = new Tools::Log::Config( %hash, default => $CONFIG ) or return undef;
...
sub _new_stack {
    ...
    my $tmpl = {
        stack   => { default        => [] },
        config  => { default        => bless( {}, 'Tools::Log::Config'),
                     required       => 1,
                     strict_type    => 1
                },
    };
    ...
    my $args = check( $tmpl, \%hash, $CONFIG->verbose )
                or (
                    warn(qq[Could not create a new stack object!]),
                    return
                );
...
The tag to add to this message. If not provided, Tools::Log will look
in your configuration for one.

=item level

The level at which this message should be handled. If not provided,
Tools::Log will look in your configuration for one.
...
sub store {
    ...
    my $args = check( $tmpl, \%hash )
            or ( warn( qq[Could not store error!] ), return );

    my $item = Tools::Log::Item->new(   %$args,
                                        parent  => $self,
                                        id      => scalar @{$self->{STACK}}
                                    )
            or ( warn( qq[Could not create new log item!] ), return undef );
...
sub retrieve {
    ...
    my $args = check( $tmpl, \%hash )
        or ( warn( qq[Could not parse input!] ), return  );
...
L<Tools::Log::Item>, L<Tools::Log::Handlers>, L<Tools::Log::Config>
