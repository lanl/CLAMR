...
sub my_get_batch_id{
    ...
    $id = $ENV{PBS_JOBID} || $ENV{SLURM_JOBID} || $ENV{SLURM_JOB_ID} || $ENV{BATCH_JOBID};
...
sub path_add_default{
    ...
    $dir = &my_dir( $0 );
    $path = "$dir";
    # loc of this file and parents
    $root1 = &my_dir( __FILE__ );
    $root2 = &my_dir( $root1 );
    $root3 = &my_dir( $root2 );
    # directory of script, directory of this file, Tools.rh/Environment, Source.rh/build
    # loc of this file
    if( $root1 ne $dir ){
        $path .= ":$root1";
    }
    # Tools.rh/Environment
...
# get a lockfile
# TYPE: 1: <UNSET> lockfile exec (might be safest, but cannot re-read lockfile to
#           get new lock time)
#       2: <DEFAULT> uses flock() seems good - tested on 20 simultaneous processes
#           says not nfs safe...
#       3: <UNSET> my homegrown in case others fail.  not safe
sub my_lockfile{
    my %args = (
                ...
                TYPE     => 2,        # 1 lockfile, 2 flock(), 3 homegrown
                RECURSE  => undef,    # if called from within my_lockfile
                WAIT     => undef,    # If just waiting for lockfile - will not create one
                WARNING  => undef,    # If error, just warn, not abort
                @_,
                );
    my $args_valid =("LOCKFILE|NOWAIT|QUIET|REASON|RECURSE|TIME|TYPE|WAIT|WARNING");
    my(
       $arg,
       $cmd,
       $done,
    ...
    # get info about lockfile
    &my_lockfile_getinfo( $lockfile, \%lockfile_info, $args{TYPE}, $time );

    if( ! defined( $args{QUIET} ) || $args{QUIET} !~ /^(|success)$/ ){
        print "my_lockfile $$ start ".&date_ymdhms_sep()." [$lockfile]\n";
        if( $lockfile_info{age} ne "" ){
    ...
    # just return if NOWAIT and lockfile is there
    if( defined($args{NOWAIT}) && $lockfile_info{age} ne "" ){
        # only return if still less than time_honor
    ...
    # wait for time to expire or lockfile to disappear
    $done = "false";
    $uname = `uname`;
    $uname =~ s/\s+//g;
    while( $done eq "false" ){
        while( $lockfile_info{age} ne "" ){
            &my_lockfile_getinfo( $lockfile, \%lockfile_info, $args{TYPE}, $time );
            if( $lockfile_info{age} eq "" ){
                last;
            }
            elsif( $lockfile_info{age} > $lockfile_info{time_honor} ){
                $forced = "$lockfile_info{age} > $lockfile_info{time_honor}";
                last;
            }
            sleep( 1 );
        }
        # now use flock to grab file
        open( $fh_FILE, "+< $lockfile" ) || open( $fh_FILE, "+> $lockfile" ) || die;
        # non-blocking so that other procs go back to pool after this proc
        # rewrites lockfile
        $flock_ret = 1;
        $flock_ret = flock( $fh_FILE, 2|8 );
        # on some machines, will always return "valse" - just turn it to ok and pray
        # could instead try a couple of times then give up and just take the lock
        # anyways.
        if( $flock_ret == 0 || $uname eq "Darwin" ){
            $done = "true";
        }
        # try again later
        else{
    ...
    # lockfile exec
    # currently, this interferes with flock - so comment out
    if( $args{TYPE} == 1 && 1 == 2 ){
        $cmd = "$lockfile_exec -s $sleep -l 1 $lockfile";
        $out = `$cmd 2>&1`;
        chmod( $lockfile_perm, $lockfile );
    }

    # home grown not very safe
    # in case lockfile and flock not available/working
    # currently, this interferes with flock - so comment out
    if( $args{TYPE} == 3 && 1 == 2 ){
        $done = "false";
        while( $done eq "false" ){
            ...
            # does not seem to work as well as other method
            # doing both seems to be best
            # recursively get another lockfile and only process with lock is done
            if( defined($args{RECURSE}) ){
                sleep 1;
            }
            # tried TYPE=1 but that seemed to sync locks up poorly
            if( ! defined( $args{RECURSE} ) ){
                $ierr = &my_lockfile( LOCKFILE=>"$lockfile.tmp", RECURSE=>"", NOWAIT=>"", QUIET=>"",
                                      REASON=>"Recursive in my_lockfile", TIME=>5, TYPE=>2 );
                if( $ierr == 0 ){
                    $done = "true";
                    next;
                }
                    ...
                    $done = "false";
                    # give enough time for file to be written
                    sleep( 2 );
                    next;
                }
            }
            ...
            # seems to work better than recursive method
            # get contents of old lockfile, sleep, then
            # get contents again.
            # if another process has modified it, then
            # hopefully this will catch it
            $done = "false";
            if( -e $lockfile ){
                $lockfile_old = `cat $lockfile 2> /dev/null`;
            }
            else{
                $lockfile_old = "";
            }
            sleep( 1 );
            if( -e $lockfile ){
                $lockfile_new = `cat $lockfile 2> /dev/null`;
            }
            else{
                $lockfile_new = "";
            }
            $lockfile_new = `cat $lockfile 2> /dev/null`;
            if( $lockfile_old eq $lockfile_new ){
                $done = "true";
            }
            else{
                sleep( 1 );
    ...
    if( ! defined( $args{QUIET} ) || $args{QUIET} !~ /^(|success)$/ ){
        if( defined( $forced ) ){
            print "my_lockfile $$ forced time ", time(), " [$forced]\n";
        }
    ...
    # remove lockfile if just WAITing
    # still write everything into lockfile in case something strange happens
    if( defined( $args{WAIT} ) ){
        unlink( $lockfile );
    }

    # remove recurse if exists
    unlink("$lockfile.tmp");
...
sub my_lockfile_getinfo{
    ...
    # if not given a time, wait forever
    if( ! defined( $time_in ) ){
        $$info_ref{time_honor} = 99999999; # default forever
    }
    else{
    ...
    foreach $line ( @lines ){
        if( $line =~ /^TIME_S:\s*(\d+)\s*$/m ){
            $$info_ref{TIME_S} = $1;
            $$info_ref{age} = time() - $$info_ref{TIME_S};
        }
        elsif( $line =~ /^TIME:\s*(\d+)\s*$/m ){
            $$info_ref{TIME} = $1;
        }
    }
    ...
    # mod time_honor depending on time_honor_file
    if( defined( $$info_ref{TIME} ) ){
        # type 1:
        # time_honor will be max of:
        #     time for this lockfile
        #     time already in lockfile
        # since gotton once and cannot re-get
        # max because Could be a race condition:
        #    p1 grabs lock (time = 10 secs)
        #    p2 and p3 race for lock (time = 1 hour)
        #    p2 sees lock honor=max(10 secs, 1 hour)
        #    p3 sees lock honor=max(10 secs, 1 hour)
        #    p2 grabs lock and needs 1 hour
        #    p3 will wait for 1 hour (might have waited for 10 secs if not "max")
        if( $type == 1 ){
            if( $$info_ref{TIME} > $$info_ref{time_honor} ){
                $$info_ref{time_honor} = $$info_ref{TIME};
            }
        }
        # will get time_honor_file again so can use min
        else{
            $$info_ref{time_honor} = $$info_ref{TIME};
        }
    }
...
sub get_pname{
  ...
  if( -e "$run_job_file" ){
      $output = `egrep 'RJ_CMD_PRUN.*\.in*' $run_job_file`;
...
#   "a02","ml9","ml2","ml1","ml2","ml3","ml5","a1" -> "a[1-2];ml[1-3,5,9]"
sub array_to_range{
    my(
        $array_ref,
        ) = @_;
...
sub my_copy_obj{
    ...
    if( ref( $obj_a_ref ) eq "HASH" ){
        &my_copy_hash( \%{$obj_a_ref}, \%{$obj_b_ref} );
    }
    elsif( ref( $obj_a_ref ) eq "ARRAY" ){
        &my_copy_hash( \@{$obj_a_ref}, \@{$obj_b_ref} );
    }
    else{
        $$obj_a_ref = $$obj_b_ref;
...
sub my_copy_hpss{
  ...
  my $args_valid =("ERROR_FILE|ERROR_SUB|PATH_FROM|PATH_TO|GROUP|PRESERVE|RM|VERBOSE");
  my(
     $arg,
     $command,
     $dir,
  ...
  &run_command( COMMAND=>$command, ERROR_REGEXP=>'/Error E/', ERROR_FILE=>$args{ERROR_FILE}, VERBOSE=>$args{VERBOSE}, ERROR_SUB=>$args{ERROR_SUB} );
...
sub my_mkdir {
    ...
    if( ! -d $dir ){
        ...
        foreach $dir ( @dirs ){
            ...
            # always add group sticky bit...sheesh - should be default
            # must be done after the chgrp
            `chmod g+s $dir_cur`;
        }
        umask($umask_old);
...
sub run_command
  {
    ...
    foreach $key ( keys %args )
      {
        if( $key !~ /^(APPEND|COMMAND|DEBUG|ERROR_FILE|ERROR_REGEXP|ERROR_STATUS|ERROR_SUB|FORK|FORK_EXEC|GROUP|OUT_FILE|STATUS|STDOUT|TIMEOUT|TIMING|VERBOSE)$/)
          {
            $ierr = 1;
...
sub which_exec {
    ...
    #...invalid args
    foreach $key ( keys %args ) {
        if( $key !~ /^(QUIET|ERROR|NOEXEC|USE|XPATH)$/) {
            $ierr = 1;
...
sub my_stat
  {
    ...
    if( $timeout_hit eq "false" && -e $file ){
        $$stat_ref{exists} = "";
        @fields = stat $file;
