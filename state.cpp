...
#pragma omp barrier
   ...
   for (int lev = mesh->levmx; lev > -1; lev--){
      ...
      for (int gix = lowerBound; gix < upperBound; gix++) {
         ...
         if (lev != mesh->level[gix]) continue;
         ...
         if ((FakeFluxHxP[gix] > 0) || (FakeFluxUxP[gix] > 0) || (FakeFluxVxP[gix] > 0)) {
            ...
            FakeFluxUxP[gix] = 0.0;
            FakeFluxVxP[gix] = 0.0;
            wplusx_H = tempWHxP[gix];
            wplusx_U = tempWUxP[gix];
            tempWHxP[gix] = 0.0;
            tempWUxP[gix] = 0.0;
         }
         if ((FakeFluxHxM[gix] > 0) || (FakeFluxUxM[gix] > 0) || (FakeFluxVxM[gix] > 0)) {
            Hxfluxminus = FakeFluxHxM[gix] * HALF; 
            Uxfluxminus = FakeFluxUxM[gix] * HALF; 
            Vxfluxminus = FakeFluxVxM[gix] * HALF; 
            FakeFluxHxM[gix] = 0.0;
            FakeFluxUxM[gix] = 0.0;
            FakeFluxVxM[gix] = 0.0;
            wminusx_H = tempWHxM[gix];
            wminusx_U = tempWUxM[gix];
            tempWHxM[gix] = 0.0;
            tempWUxM[gix] = 0.0;
         }
         if ((FakeFluxHyP[gix] > 0) || (FakeFluxUyP[gix] > 0) || (FakeFluxVyP[gix] > 0)) {
            Hyfluxplus = FakeFluxHyP[gix] * HALF; 
            Uyfluxplus = FakeFluxUyP[gix] * HALF; 
            Vyfluxplus = FakeFluxVyP[gix] * HALF; 
            FakeFluxHyP[gix] = 0.0;
            FakeFluxUyP[gix] = 0.0;
            FakeFluxVyP[gix] = 0.0;
            wplusy_H = tempWHyP[gix];
            wplusy_V = tempWVyP[gix];
            tempWHyP[gix] = 0.0;
            tempWVyP[gix] = 0.0;
         }
         if ((FakeFluxHyM[gix] > 0) || (FakeFluxUyM[gix] > 0) || (FakeFluxVyM[gix] > 0)) {
            Hyfluxminus = FakeFluxHyM[gix] * HALF; 
            Uyfluxminus = FakeFluxUyM[gix] * HALF; 
            Vyfluxminus = FakeFluxVyM[gix] * HALF; 
            FakeFluxHyM[gix] = 0.0;
            FakeFluxUyM[gix] = 0.0;
            FakeFluxVyM[gix] = 0.0;
            wminusy_H = tempWHyM[gix];
            wminusy_V = tempWVyM[gix];
            tempWHyM[gix] = 0.0;
            tempWVyM[gix] = 0.0;
         }
...
#pragma omp for 
   ...
   for (int iface = 0; iface < mesh->nxface; iface++){
      int cell_lower = mesh->map_xface2cell_lower[iface];
      int cell_upper = mesh->map_xface2cell_upper[iface];
      if (mesh->level[cell_lower] == mesh->level[cell_upper]) {
         ...
         int lev = mesh->level[cell_lower];
         int levr = mesh->level[cell_upper];
         real_t dxic = mesh->lev_deltax[lev]; 
         real_t dxr = mesh->lev_deltax[levr];
         ...
         int nrt = mesh->ntop[cell_upper];
         int nrtr = 0;
         real_t Hrt = 0.0; 
         real_t Urt = 0.0;
         real_t Hrr2 = 0.0;
         real_t Urr2 = 0.0;
         if(lev < mesh->level[cell_upper]) {
            nrtr = mesh->nrht[nrt];
            Hrt  = H[ mesh->ntop[cell_upper] ];
            Urt  = U[ mesh->ntop[cell_upper] ];
            Hrr2 = H[nrtr];
            Urr2 = U[nrtr];
         }
         ...
         Wx_H[iface] = w_corrector(deltaT, dx_avg, U_eigen, Hr-Hic, Hic-Hl2, Hrr-Hr) * (Hr - Hic) * HALF;
         Wx_U[iface] = w_corrector(deltaT, dx_avg, U_eigen, Ur-Uic, Uic-Ul2, Urr-Ur) * (Ur - Uic) * HALF;
 
         if(lev < level[cell_upper]) {
            if(level[nrt] < level[nrtr])
               Hrr2 = (Hrr2 + H[ mesh->ntop[nrtr] ]) * HALF,
               Urr2 = (Urr2 + U[ mesh->ntop[nrtr] ]) * HALF;
            Wx_H[iface] = w_corrector(deltaT, dx_avg, U_eigen, Hrt-Hic, Hic-Hl2, Hrr2-Hrt) *
                                     (Hrt - Hic) * HALF;
            Wx_U[iface] = w_corrector(deltaT, dx_avg, U_eigen, Urt-Uic, Uic-Ul2, Urr2-Urt) *
                                     (Urt - Uic) * HALF;
         }
      }
   }
...
#pragma omp for 
   ...
   for (int iface = 0; iface < mesh->nyface; iface++){
      ...
      if (mesh->level[cell_lower] == mesh->level[cell_upper]) {
         ...
         int lev = mesh->level[cell_lower];
         int levt = mesh->level[cell_upper];
         real_t dyic = mesh->lev_deltay[lev];
         real_t dyt = mesh->lev_deltay[levt];
         ...
         real_t V_eigen = fabs(Vy[iface]/Hy[iface]) + sqrt(g*Hy[iface]);
         real_t dy_avg = (dyic+dyt)*HALF; 

         int ntr = mesh->nrht[cell_upper];
         int ntrt = 0;
         real_t Htr = 0.0;
         real_t Vtr = 0.0; 
         real_t Htt2 = 0.0;
         ...
         if(lev < mesh->level[cell_upper]) {
            ntrt = mesh->ntop[ntr];
            Htr  = H[ mesh->nrht[cell_upper] ];
            Vtr  = V[ mesh->nrht[cell_upper] ];
            Htt2 = H[ntrt];
            Vtt2 = V[ntrt];
         } 
         ...
         Wy_H[iface] = w_corrector(deltaT, dy_avg, V_eigen, Ht-Hic, Hic-Hb2, Htt-Ht) * (Ht - Hic) * HALF;
         Wy_V[iface] = w_corrector(deltaT, dy_avg, V_eigen, Vt-Vic, Vic-Vb2, Vtt-Vt) * (Vt - Vic) * HALF;
 
         if(lev < level[cell_upper]) {
            if(level[ntr] < level[ntrt])
               Htt2 = (Htt2 + H[ mesh->nrht[ntrt] ]) * HALF,
               Vtt2 = (Vtt2 + V[ mesh->nrht[ntrt] ]) * HALF;
            Wy_H[iface] = w_corrector(deltaT, dy_avg, V_eigen, Htr-Hic, Hic-Hb2, Htt2-Htr) *
                                     (Htr - Hic) * HALF;
            Wy_V[iface] = w_corrector(deltaT, dy_avg, V_eigen, Vtr-Vic, Vic-Vb2, Vtt2-Vtr) *
                                     (Vtr - Vic) * HALF;
         }
      }
   }
...
#pragma omp barrier
   ...
   for (int lev = mesh->levmx; lev > -1; lev--){
      ...
      for (int ic = lowerBound; ic < upperBound; ic++){
         ...
         //XXX should these be mult. by HALF?
         if(fl2 >= 0){
            Hxminus = (Hxminus + Hx[fl2]);
            Uxminus = (Uxminus + Ux[fl2]);
            Vxminus = (Vxminus + Vx[fl2]);
         }
         ...
         //XXX should these be mult. by HALF?
         if(fr2 >= 0){
            Hxplus = (Hxplus + Hx[fr2]);
            Uxplus = (Uxplus + Ux[fr2]);
            Vxplus = (Vxplus + Vx[fr2]);
         }
         ...
         //XXX should these be mult. by HALF?
         if(fl2 >= 0){
            wminusx_H = (wminusx_H + Wx_H[fl2]);
            wminusx_U = (wminusx_U + Wx_U[fl2]);
         }
         ...
         //XXX should these be mult. by HALF?
         if(fr2 >= 0){
            wplusx_H = (wplusx_H + Wx_H[fr2]);
            wplusx_U = (wplusx_U + Wx_U[fr2]);
         }
         ...
         //XXX should these be mult. by HALF?
         if(fb2 >= 0){
            Hyminus = (Hyminus + Hy[fb2]);
            Uyminus = (Uyminus + Uy[fb2]);
            Vyminus = (Vyminus + Vy[fb2]);
         }
         ...
         //XXX should these be mult. by HALF?
         if(ft2 >= 0){
            Hyplus = (Hyplus + Hy[ft2]);
            Uyplus = (Uyplus + Uy[ft2]);
            Vyplus = (Vyplus + Vy[ft2]);
         }
         ...
         if(fb2 >= 0){
         //XXX should these be mult. by HALF?
            wminusy_H = (wminusy_H + Wy_H[fb2]);
            wminusy_V = (wminusy_V + Wy_V[fb2]);
         }
         ...
         //XXX should these be mult. by HALF?
         if(ft2 >= 0){
            wplusy_H = (wplusy_H + Wy_H[ft2]);
            wplusy_V = (wplusy_V + Wy_V[ft2]);
         }
...
void State::calc_finite_difference_regular_cells(double deltaT){
   ...
   //double ***FakeFluxHyM, ***FakeFluxUyM, ***FakeFluxVyM;
   //double ***tempWHxP, ***tempWHxM, ***tempWHyP, ***tempWHyM;
   //double ***tempWUxP, ***tempWUxM, ***tempWVyP, ***tempWVyM;
    ...
    * 4 = wminus[x/y]_L
    * 5 = wplus[x/y]_L
    * 6 & 7 are only use for H which uses both x and y for w corrector
    * */
...
#pragma omp master
   {
      ...
      FakeFluxHyP = (double ***) malloc((mesh->levmx+1) * sizeof(double **));
      FakeFluxHyM = (double ***) malloc((mesh->levmx+1) * sizeof(double **));
      FakeFluxUxP = (double ***) malloc((mesh->levmx+1) * sizeof(double **));
      FakeFluxUxM = (double ***) malloc((mesh->levmx+1) * sizeof(double **));
      FakeFluxUyP = (double ***) malloc((mesh->levmx+1) * sizeof(double **));
      FakeFluxUyM = (double ***) malloc((mesh->levmx+1) * sizeof(double **));
      FakeFluxVxP = (double ***) malloc((mesh->levmx+1) * sizeof(double **));
      FakeFluxVxM = (double ***) malloc((mesh->levmx+1) * sizeof(double **));
      FakeFluxVyP = (double ***) malloc((mesh->levmx+1) * sizeof(double **));
      FakeFluxVyM = (double ***) malloc((mesh->levmx+1) * sizeof(double **));
      tempWHxP = (double ***) malloc((mesh->levmx+1) * sizeof(double **));
      tempWHxM = (double ***) malloc((mesh->levmx+1) * sizeof(double **));
      tempWHyP = (double ***) malloc((mesh->levmx+1) * sizeof(double **));
      tempWHyM = (double ***) malloc((mesh->levmx+1) * sizeof(double **));
      tempWUxP = (double ***) malloc((mesh->levmx+1) * sizeof(double **));
      tempWUxM = (double ***) malloc((mesh->levmx+1) * sizeof(double **));
...
/*
          FakeFluxHxP[lev] = (double **)genmatrix(mesh->lev_jregsize[lev],mesh->lev_iregsize[lev],sizeof(double));
          FakeFluxHxM[lev] = (double **)genmatrix(mesh->lev_jregsize[lev],mesh->lev_iregsize[lev],sizeof(double));
          FakeFluxHyP[lev] = (double **)genmatrix(mesh->lev_jregsize[lev],mesh->lev_iregsize[lev],sizeof(double));
          FakeFluxHyM[lev] = (double **)genmatrix(mesh->lev_jregsize[lev],mesh->lev_iregsize[lev],sizeof(double));
          FakeFluxUxP[lev] = (double **)genmatrix(mesh->lev_jregsize[lev],mesh->lev_iregsize[lev],sizeof(double));
          FakeFluxUxM[lev] = (double **)genmatrix(mesh->lev_jregsize[lev],mesh->lev_iregsize[lev],sizeof(double));
          FakeFluxUyP[lev] = (double **)genmatrix(mesh->lev_jregsize[lev],mesh->lev_iregsize[lev],sizeof(double));
          FakeFluxUyM[lev] = (double **)genmatrix(mesh->lev_jregsize[lev],mesh->lev_iregsize[lev],sizeof(double));
          FakeFluxVxP[lev] = (double **)genmatrix(mesh->lev_jregsize[lev],mesh->lev_iregsize[lev],sizeof(double));
          FakeFluxVxM[lev] = (double **)genmatrix(mesh->lev_jregsize[lev],mesh->lev_iregsize[lev],sizeof(double));
          FakeFluxVyP[lev] = (double **)genmatrix(mesh->lev_jregsize[lev],mesh->lev_iregsize[lev],sizeof(double));
          FakeFluxVyM[lev] = (double **)genmatrix(mesh->lev_jregsize[lev],mesh->lev_iregsize[lev],sizeof(double));
          tempWHxP[lev] = (double **)genmatrix(mesh->lev_jregsize[lev],mesh->lev_iregsize[lev],sizeof(double));
          tempWHxM[lev] = (double **)genmatrix(mesh->lev_jregsize[lev],mesh->lev_iregsize[lev],sizeof(double));
          tempWHyP[lev] = (double **)genmatrix(mesh->lev_jregsize[lev],mesh->lev_iregsize[lev],sizeof(double));
          tempWHyM[lev] = (double **)genmatrix(mesh->lev_jregsize[lev],mesh->lev_iregsize[lev],sizeof(double));
          tempWUxP[lev] = (double **)genmatrix(mesh->lev_jregsize[lev],mesh->lev_iregsize[lev],sizeof(double));
   ...
   for (int ll=mesh->levmx; ll>-1; ll--){
      ...
      for(int jj=2; jj<jjmax-2; jj++){
         for(int ii=2; ii<iimax-2; ii++){
            ...
            //printf("%d/%d %d ", jj, ii, passFlag[ll][jj][ii*4+0]);
...
//#ifdef XXX
            ...
            real_t Hyminus = HALF*( ((H_reg[jj-1][ii]) + (H_reg[jj][ii])) - (deltaT)/(dy)*((HYRGFLUXNB) - (HYRGFLUXIC)) );
            real_t Uyminus = HALF*( ((U_reg[jj-1][ii]) + (U_reg[jj][ii])) - (deltaT)/(dy)*((UYRGFLUXNB) - (UYRGFLUXIC)) );
            real_t Vyminus = HALF*( ((V_reg[jj-1][ii]) + (V_reg[jj][ii])) - (deltaT)/(dy)*((VYRGFLUXNB) - (VYRGFLUXIC)) );

            real_t Hyplus  = HALF*( ((H_reg[jj][ii]) + (H_reg[jj+1][ii])) - (deltaT)/(dy)*((HYRGFLUXIC) - (HYRGFLUXNT)) );
            real_t Uyplus  = HALF*( ((U_reg[jj][ii]) + (U_reg[jj+1][ii])) - (deltaT)/(dy)*((UYRGFLUXIC) - (UYRGFLUXNT)) );
            real_t Vyplus  = HALF*( ((V_reg[jj][ii]) + (V_reg[jj+1][ii])) - (deltaT)/(dy)*((VYRGFLUXIC) - (VYRGFLUXNT)) );
...
#pragma omp barrier
   ...
   for (int ll = mesh->levmx; ll > -1; ll--){
      ...
      for(int jj=2; jj<jjmax-1; jj++){
         for(int ii=2; ii<iimax-1; ii++){
            ...
            if (mask_reg[jj][ii-1] == 1 || mask_reg[jj][ii] == 1){
               real_t Hx = HALF*( (H_reg[jj][ii-1] + H_reg[jj][ii]) - deltaT/dx*(HXRGFLUXIC - HXRGFLUXNL) );
               real_t Ux = HALF*( (U_reg[jj][ii-1] + U_reg[jj][ii]) - deltaT/dx*(UXRGFLUXIC - UXRGFLUXNL) );
               real_t Vx = HALF*( (V_reg[jj][ii-1] + V_reg[jj][ii]) - deltaT/dx*(VXRGFLUXIC - VXRGFLUXNL) );
            ...
            if (mask_reg[jj-1][ii] == 1 || mask_reg[jj][ii] == 1){
               real_t Hy = HALF*( (H_reg[jj-1][ii] + H_reg[jj][ii]) - deltaT/dy*(HYRGFLUXIC - HYRGFLUXNB) );
               real_t Uy = HALF*( (U_reg[jj-1][ii] + U_reg[jj][ii]) - deltaT/dy*(UYRGFLUXIC - UYRGFLUXNB) );
               real_t Vy = HALF*( (V_reg[jj-1][ii] + V_reg[jj][ii]) - deltaT/dy*(VYRGFLUXIC - VYRGFLUXNB) );
      ...
      for (int jj = 2; jj < jjmax - 1; jj++) {
         for (int ii = 2; ii < iimax-1; ii++) {
            if (mask_reg[jj][ii-1] == 1 || mask_reg[jj][ii] == 1) {
                ...
                printf("%d %d/%d -> %d/%d [%d %d]\n", ll, jj, ii, recvJ, recvI, mesh->lev_jregmin[ll-1], mesh->lev_iregmin[ll-1]);
      ...
      for (int iface = 0; iface < mesh->nxface; iface++){
         ...
         if (recvIdx < 0) continue;//{
         //printf("1st pass x direction recvIdx %d iface %d i %d j %d lev %d nzlower %d nzupper %d\n",
         //   recvIdx, iface, mesh->xface_i[iface], mesh->xface_j[iface], mesh->xface_level[iface],
         //   mesh->map_xface2cell_lower[iface], mesh->map_xface2cell_upper[iface]);
         //}

         int ii = mesh->xface_i[iface] - mesh->lev_iregmin[ll];
         int jj = mesh->xface_j[iface] - mesh->lev_jregmin[ll];
         int recvJ = mesh->xface_j[recvIdx] - mesh->lev_jregmin[ll-1];
         int recvI = mesh->xface_i[recvIdx] - mesh->lev_iregmin[ll-1];

         printf("%d (%d) %d/%d (%d) %d/%d\n", ll, iface, jj, ii, recvIdx, recvJ, recvI);
         //printf("\t%d) %d %d %d\n", iface, ll, mesh->xface_j[iface], mesh->xface_i[iface]);
         HxFlux[ll-1][recvJ][recvI] += HxFlux[ll][jj][ii] *HALF;
         UxFlux[ll-1][recvJ][recvI] += UxFlux[ll][jj][ii] *HALF;
         VxFlux[ll-1][recvJ][recvI] += VxFlux[ll][jj][ii] *HALF;
         Wx_H[ll-1][recvJ][recvI] += Wx_H[ll][jj][ii] / 4;
      ...
      for (int iface = 0; iface < mesh->nyface; iface++){
         ...
         //   mesh->map_xface2cell_lower[iface], mesh->map_xface2cell_upper[iface]);
         if (recvIdx < 0) continue;//{
            //printf("1st pass x direction recvIdx %d i %d j %d lev %d nzlower %d nzupper %d\n",
            //   recvIdx, mesh->xface_i[recvIdx], mesh->xface_j[recvIdx], mesh->xface_level[recvIdx],
            //   mesh->map_xface2cell_lower[recvIdx], mesh->map_xface2cell_upper[recvIdx]);
         //}

         int ii = mesh->yface_i[iface] - mesh->lev_iregmin[ll];
         int jj = mesh->yface_j[iface] - mesh->lev_jregmin[ll];
      ...
      for(int jj=2; jj<jjmax-2; jj++){
         for(int ii=2; ii<iimax-2; ii++){
            ...
            //mass conservation by flux transfer
            if ((FakeFluxHxP[ll][jj][ii] > 0) || (FakeFluxUxP[ll][jj][ii] > 0) || (FakeFluxVxP[ll][jj][ii] > 0)) {
               printf("DEBUG coarse receiver lev %d j %d i %d H %lf\n",ll,jj,ii,FakeFluxHxP[ll][jj][ii]);
               Hxfluxplus = FakeFluxHxP[ll][jj][ii] * HALF; 
               Uxfluxplus = FakeFluxUxP[ll][jj][ii] * HALF;
               Vxfluxplus = FakeFluxVxP[ll][jj][ii] * HALF; 
               FakeFluxHxP[ll][jj][ii] = 0.0;
            ...
            if ((mesh->phantomXFluxRG[ll][jj][ii] >= 0) && (mesh->phantomXFluxRG[ll][jj][ii] < 99999)) {
               ...
               int recvJ = mesh->j[recvic] - mesh->lev_jregmin[ll-1];
               int recvI = mesh->i[recvic] - mesh->lev_iregmin[ll-1];
               printf("DEBUG fine sender lev %d j %d i %d recvic %d recvJ %d recvI %d H %lf\n",ll,jj,ii,recvic,recvJ,recvI,FakeFluxHxP[ll][jj][ii]);
#ifdef _OPENMP
#pragma omp atomic update
#endif
               FakeFluxHxP[ll-1][recvJ][recvI] += Hxfluxminus;
...
#pragma omp atomic update
            ...
            /*H_reg_new[jj][ii] = H_reg[jj][ii] - Cx*(HNEWXRGFLUXFL - HNEWXRGFLUXFR)
                                                 -wminusx*(H_reg[jj][ii]-H_reg[jj][ii-1])
                                                 +wplusx*(H_reg[jj][ii+1]-H_reg[jj][ii])
                                              - Cy*(HNEWYRGFLUXFB - HNEWYRGFLUXFT),
                                                 -wminusy*(H_reg[jj][ii]-H_reg[jj-1][ii])
                                                 +wplusy*(H_reg[jj+1][ii]-H_reg[jj][ii]);
            U_reg_new[jj][ii] = U_reg[jj][ii] - Cx*(UNEWXRGFLUXFL - UNEWXRGFLUXFR)
                                                 -wminusx*(U_reg[jj][ii]-U_reg[jj][ii-1])
                                                 +wplusx*(U_reg[jj][ii+1]-U_reg[jj][ii])
                                              - Cy*(UNEWYRGFLUXFB - UNEWYRGFLUXFT),
                                                 -wminusy*(U_reg[jj][ii]-U_reg[jj-1][ii])
                                                 +wplusy*(U_reg[jj+1][ii]-U_reg[jj][ii]);
            V_reg_new[jj][ii] = V_reg[jj][ii] - Cx*(VNEWXRGFLUXFL - VNEWXRGFLUXFR)
                                                 -wminusx*(V_reg[jj][ii]-V_reg[jj][ii-1])
                                                 +wplusx*(V_reg[jj][ii+1]-V_reg[jj][ii])
                                              - Cy*(VNEWYRGFLUXFB - VNEWYRGFLUXFT),
                                                 -wminusy*(V_reg[jj][ii]-V_reg[jj-1][ii])
                                                 +wplusy*(V_reg[jj+1][ii]-V_reg[jj][ii]);*/
      ...
      genmatrixfree((void **)HxFlux[0]);
      genmatrixfree((void **)UxFlux[0]);
